...
export const fetchDocs = async <T>(
  ...
  draft?: boolean,
  limit = 300, // Default limit value
  sort: "desc" | "asc" = "desc", // Default order value
): Promise<T[]> => {
  ...
  const docs: T[] = await fetch(`${GRAPHQL_API_URL}/api/graphql`, {
    ...
    body: JSON.stringify({
      ...
      variables: {
        limit,
        sort: sort === "desc" ? "-createdAt" : "createdAt", // sort by either descendent or ascendent, https://payloadcms.com/docs/queries/overview#sort
      },
    }),
  })
    ?.then(res => res.json())
